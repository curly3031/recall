        ...
        with connect(
            ...
            with connection.cursor() as cursor:
                query = f"SELECT calldate, clid, src, dst, disposition, uniqueid, processed, is_second, is_third FROM " \
                        f"cdr WHERE LENGTH(src) <= 4 and disposition !='ANSWERED' and is_second=0 and calldate LIKE " \
                ...
                for row in result:
                    ...
                    # if uniqueid:
                    #     update_query = f"UPDATE cdr set is_second=1 WHERE uniqueid LIKE'%{orig_uniqueid}%' "
